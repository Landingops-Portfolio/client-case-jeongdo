...
var RENDER = {
    init: function (property) {
					...
					stickyElements.forEach((stickyElement) => {
						...
						if(input_gjs.length == 0) {
							if(window.scrollY >= stickyElement.offsetTop) {
								...
								if(!isSidebar) stickyElement.style.top = `${document.querySelector('header').clientHeight}px`;
		...
		var moveLinkPage = function(link,interval,isblock) {
			...
			var $aosEl = $('.'+link + '[data-aos]');

			if( $aosEl.length > 0 && !$aosEl.hasClass('aos-animate')) {
				console.log("test");
				const tr  = $aosEl.css("transform");
				let moveY = 0;

				if (tr && tr !== "none") {
					const vals = tr.match(/-?\d+\.?\d*/g);
					moveY = parseFloat(vals[13] || vals[5] || 0); // 이미 음수/양수 포함됨
				
					sTop -= moveY;
				  }
				   
			}
			var duration = (typeof $('#tpl-menu').attr('data-duration') != 'undefined') ? parseInt($('#tpl-menu').attr('data-duration')) : 1000;
				...
				if(typeof val.webfonts != 'undefined' && val.webfonts) {
					$.each(val.webfonts, function(i2,v2) {
						if(b.ELFONTS.indexOf(i2) < 0 && v2) {
							b.ELFONTS.push(i2);
							WebFont.load({ google : { families : [ v2 ], display:'swap' }});
						}
					})
				}
							...
							if( i.length>0 || (i.length==0 && is_gc_cookie) ) {
								...
								$.each(data.list,function(index,v) {
									...
									if(checkGallerySVG) {
										...
										gimgSVG.find('polygon, path').attr('fill', 'url(#gimgSvg_'+val.elname+'_'+index+')');
									}
		...
		$('*[data-user-link]').live('click',function(e) {
			...
			userLink = ($(this).parents('.element').hasClass('logoSlide') && isshlink) ?  $(this).attr('data-shlink') : userLink;
		...
		let calendarElsettings = new Map();
		let googleColor = new Map();
			...
			init : function(sid, pid, yearMonth, c) { 
				...
				if(data_type != '') {
					$.ajax({
						...
						data: { sid: sid, pid: pid, yearMonth: ym, data_type: data_type },
						type: 'POST',
						dataType: 'json',
						success: function (data) {
							checkError(data);
							var inst = $.cl_calendar.instances[pid];
							...
							// --- 모바일 이벤트 렌더링 (선택 월 기준) ---
							var $root = $('.element[data-id="' + pid + '"]');
							if ($root.find('.mobile-events').length === 0) {
								$root.find('.notice').before('<div class="mobile-events"></div>');
							}
							var $mobile = $root.find('.mobile-events').empty();

							var currentYear = inst.currentDate.getFullYear();
							var currentMonth = inst.currentDate.getMonth() + 1;

							var countThisMonth = 0;
							var eventMap = {};

							$.each(dataArr, function (i, v) {
								var d = new Date(v.s_date);
								var year = d.getFullYear();
								var month = d.getMonth() + 1;

								if (year === currentYear && month === currentMonth) {
									countThisMonth++;
									var dateKey = v.s_date; // 'YYYY-MM-DD'
									var time = (v.fulltime === 'Y') ? '종일' : (v.s_time + ' ~ ' + v.e_time);
									if (!eventMap[dateKey]) eventMap[dateKey] = [];
									eventMap[dateKey].push(
										'<div class="event-memo"><span class="event-time">' + time + '</span>' + (v.memo || '') + '</div>'
									);
								}
							});
							...
							if (countThisMonth === 0) {
								...
								var sortedDates = Object.keys(eventMap).sort();
								$.each(sortedDates, function (i, date) {
									html += '<div class="event-date">' + date_string(date) + '</div>';
									$.each(eventMap[date], function (j, memoHtml) { html += memoHtml; });
									html += '<br>';
								});
							}
					...
					calendarElsettings.set(pid, calendarShow(elsettings));
					fetchMonth(pid, ym, data_type)
						.then(function(evts) {
		...
		// 날짜/시간선택 ampm
		$(document).off('click.clockToggle').on('click.clockToggle', '.calendar .calendarModal .timeselector .select:not(.ampm) .selector', function(e){
			e.preventDefault();
			e.stopPropagation();
			...
			// ★ 드롭 닫힘을 잠시 무력화하는 락
			$field.data('dropOpenLock', true);
			setTimeout(()=>{ $field.removeData('dropOpenLock'); }, 150);
		...
		function applyEndTimeDisables($endField){
			...
			// 1) minDate는 '날짜 단위'로, '변경될 때만' 세팅 (불필요한 리렌더 방지)
			if (dpEnd && typeof dpEnd.minDate === 'function') {
				const wantMin = start.clone().startOf('day');
				let curMin = null;
				try { curMin = dpEnd.minDate && dpEnd.minDate(); } catch(e){}
				if (!curMin || !moment.isMoment(curMin) || !curMin.isSame(wantMin, 'day')) {
				dpEnd.minDate(wantMin);
				}
			}
			...
			// 2) equal-case 강제 리페인트는 '시간/분 드롭이 열려 있을 때만'
			//    (= 날짜(day) 셀 클릭 중엔 절대 실행하지 않음)
			const timeDropOpen = $endField.find('.custom-clock .select .drop:visible').length > 0;
			if (dpEnd && end && start && end.isSame(start) && timeDropOpen) {
				$endField.data('suppressDropAutoHide', true);
				if (typeof dpEnd.viewDate === 'function') dpEnd.viewDate(end.clone());
				dpEnd.date(end.clone().add(1,'ms'));
				dpEnd.date(end.clone());
				setTimeout(() => { $endField.removeData('suppressDropAutoHide'); }, 80);
			}
			...
			.on('click.clockBtns', '.calendar-period-save', function () {
				...
				if ($hostField.hasClass('sdate')) {
					...
					if (!end || m.isAfter(end)) {
						...
						const newEnd = isAllDay ? m.clone() : m.clone();
				...
				} else if ($hostField.hasClass('edate')) {
					const $startField = $overlay.find('.modal-field.sdate');
					const $startInput = $startField.find('input.daytime');
					const dpStartInstance = getDp($startField, $startInput);   // sdate
					const $endField = $overlay.find('.modal-field.edate');
					const $endInput = $endField.find('input.daytime');
					const dpEnd = dpStart; // hostField=edate

					// 1) 시작(moment) 읽기
					let start = dpStartInstance && dpStartInstance.date() ? dpStartInstance.date().clone() : null;
					if (!start) {
						const rawStart = $startInput.attr('data-raw') || $startInput.val();
						if (rawStart) start = moment(parseToDate(rawStart));
					}

					// 2) end 후보를 "커스텀 시계 선택값"으로 직접 조립
					const isAllDay = isAllDayByField($hostField);
					const gran = isAllDay ? 'day' : 'minute';
					const norm = d => d ? d.clone().seconds(0).milliseconds(0) : null;

					let endCandidate;
					if (isAllDay) {
						// 종일은 날짜만 비교하면 되므로, dp의 날짜(또는 start)로 조립
						endCandidate = (dpEnd && dpEnd.date ? dpEnd.date().clone() : (start || moment())).startOf('day');
					} else {
						// 커스텀 시계에서 AM/PM, 시, 분 읽기
						const $clock   = $hostField.find('.custom-clock');
						const ap       = ($clock.find('.select.ampm .selector').attr('data-type') || 'am').toLowerCase();
						const h12Str   = $clock.find('.select.hour .selector').text().trim() || '12';
						const minStr   = $clock.find('.select.minute .selector').text().trim() || '00';
						const toH24    = (h12, ap) => ap==='pm' ? (h12%12)+12 : (h12%12);
						const h24      = toH24(parseInt(h12Str,10)||12, ap);
						const mm       = Math.max(0, Math.min(59, parseInt(minStr,10)||0));

						// 같은 "날짜" 위에 시간만 덮어쓰기 (dpEnd가 있으면 그 날짜 사용, 없으면 start 날짜 사용)
						const base = (dpEnd && dpEnd.date ? dpEnd.date().clone() : (start || moment()));
						endCandidate = base.clone().hours(h24).minutes(mm).seconds(0).milliseconds(0);
					}

					const startNorm    = norm(start);
					const endNorm      = norm(endCandidate);

					// 3) 저장 시점 보정: end < start면 end = start
					const correctedEnd = (startNorm && endNorm && endNorm.isBefore(startNorm, gran))
						? startNorm.clone()
						: endNorm.clone();

					// 4) 위젯/뷰 갱신은 correctedEnd 기준
					if (dpEnd && typeof dpEnd.date === 'function') {
						$hostField.data('suppressHide', true);
						if (typeof dpEnd.minDate === 'function') dpEnd.minDate(startNorm ? startNorm.clone().startOf('day') : false);
						if (typeof dpEnd.viewDate === 'function') dpEnd.viewDate(correctedEnd.clone());
						dpEnd.date(correctedEnd.clone()); // (필요시 1ms 흔들기 추가 가능)
						setTimeout(() => { $hostField.removeData('suppressHide'); }, 0);
					}

					// 5) 표기/저장도 correctedEnd만 사용
					const rawStrEnd   = isAllDay ? correctedEnd.format('YYYY-MM-DD')
												: correctedEnd.format('YYYY-MM-DD HH:mm');
					const dispEndHTML = formatDateLocalized(rawStrEnd, locale, 1);
					const dispEndTEXT = formatDateLocalized(rawStrEnd, locale);

					$endInput.attr('data-raw', rawStrEnd).val(dispEndTEXT).removeClass('empty');
					const $endView = $hostField.find('.daytime-html');
					if ($endView.length) $endView.attr('data-raw', rawStrEnd).html(dispEndHTML);

					try { applyEndTimeDisables($hostField); } catch(e){}
					try { if (typeof paintActiveDay === 'function') paintActiveDay($hostField, correctedEnd.clone()); } catch(e){}
		...
		$(document).on('click', '.calendar .modal-field .daytime', function(e) {
			...
			$field.datetimepicker(periodOptions)
				...
				.on('dp.change', function(){
					const $field = $(this);
					const locale = $field.closest('.calendar').hasClass('en') ? 'en' : 'ko';

					setTimeout(() => {
						ensureCustomClock($field, locale);
						if ($field.hasClass('edate')) applyEndTimeDisables($field);

						// ★ 프로그램적 변경(equals 리페인트) or 드롭 방금 열림 → 닫기 스킵
						if ($field.data('suppressDropAutoHide') || $field.data('dropOpenLock')) return;
				...
				$.ajax({
					...
					success: function(data) {
						...
						if (data_type !== '') {
							...
							// ✅ 핵심: 수정 영향권(전/당/다음달)만 캐시 무효화
							invalidateAround(pid, yearMonth);

							//서버가 돌려준 최신 data(3개월치)를 즉시 캐시에 병합
							upsertMonthCacheFromServer(pid, data);

							// ✅ 동적 events 소스를 유지한 채로 최신 데이터 다시 요청
							if (instance && instance.calendar) {
								const v = instance.calendar.view;
								const needKeys = monthsInRange(v.currentStart, v.currentEnd);
								const pidMap = monthCache.get(pid) || new Map();
								const missing = needKeys.filter(k => !pidMap.has(k));
								const dt = ($('.userEL'+pid).attr('data-modoo-type')) || '';
								const after = () => instance.calendar.refetchEvents();

								if (missing.length) {
									Promise.all(missing.map(k => fetchMonth(pid, k, dt))).finally(after);
								} else {
									after();
								}
							} else {
		...
		$(document).on('click', '.calendar .modal-save, .calendar .modal-modify', function(e) {
			...
			var title = $btn.closest('.calendar-modal-container').find('.modal-title .ctitle').val(),
                color = $btn.closest('.calendar-modal-container').find('.scheduleColorPicker').attr('data-color'),
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data_type != '') {
						...
						// ✅ 핵심: 수정 영향권(전/당/다음달)만 캐시 무효화
						invalidateAround(pid, sdate.substring(0, 7));
						...
						if (instance && instance.calendar) {
							...
							const dt = ($('.userEL'+pid).attr('data-modoo-type')) || '';
							if (missing.length) {
								Promise.all(missing.map(k => fetchMonth(pid, k, dt)))
								.finally(() => instance.calendar.refetchEvents());
							} else {
		...
		function showCalendarModalView(pid, data, fromList) {
			...
			var isGoogle = !!(data.extendedProps && data.extendedProps.isGoogle);
			let fulltime = data.extendedProps.fulltime;
			if (isGoogle && (fulltime === undefined || fulltime === '' || fulltime === null)) {
				// extendedProps에 없으면 event 객체의 allDay로 다시 보정
				fulltime = data.allDay ? 'Y' : 'N';
			}

			let timeShow = '', startShow = '', endShow = '', rawStart = '', rawEnd = '';
			if (data.allDay) {
				if (data.extendedProps.fulltime === 'Y') { // 종일 범위
					rawStart = data.extendedProps.start_date;          // "YYYY-MM-DD"
                    rawEnd   = data.extendedProps.end_date;            // "YYYY-MM-DD"
			...
			if (!isGoogle) {
				...
				`;
				textTitle = data._def.title;
			} else {
				textTitle = (data._def.title == 'undefined') ? (locale == 'ko') ? '(제목 없음)' : '(No title)' : data._def.title;
			}
		...
		function fmtHMFromDate(d, locale) {
			if (!d) return '';
			const hour = d.getHours();
			const minute = String(d.getMinutes()).padStart(2,'0');
			if (locale === 'ko') {
				const period = hour < 12 ? $.lang[locale]['config.calendar.am'] : $.lang[locale]['config.calendar.pm'];
		...
		function openMoreEventsModal(pid, events, weekday) {
			...
			const googleColorRaw = (googleColor && googleColor.get(pid)) || '';
    		const googleColorHex = googleColorRaw ? (googleColorRaw.startsWith('#') ? googleColorRaw : ('#' + googleColorRaw)) : ''; 
			...
			for (let i = 0; i < sorted.length; i++) {
				var ev = sorted[i];
				const locale = ($('.userEL'+pid).hasClass('en')) ? 'en' : 'ko';

				let sdate = ev.extendedProps?.start_date || '';
				let edate = ev.extendedProps?.end_date || '';
				let fulltimeFlag = ev.extendedProps?.fulltime; // 'Y' 또는 'N' 기대

				const isAllDay = (fulltimeFlag === 'Y') || (!!ev.allDay);

				if (!sdate && ev.start) sdate = toDateStr(ev.start);
				...
				if (!edate) {
					if (ev.end) {
						if (isAllDay) {
							...
							edate = toDateStr(ev.end);
						}
					} else {
						edate = sdate; // 끝이 없으면 시작과 동일
					}
				}
				...
				if (sdate !== edate) {
					if (isAllDay) {
						schedule_time = `${sdate.slice(5)} ~ ${edate.slice(5)}`;
					} else {
						schedule_time = `${sdate.slice(5)} ${fmtHMFromDate(ev.start, locale)} ~ ${edate.slice(5)} ${fmtHMFromDate(ev.end, locale)}`;
					}
				} else {
					schedule_time = isAllDay ? '' : `${fmtHMFromDate(ev.start, locale)} ~ ${fmtHMFromDate(ev.end, locale)}`;
				}

				const isFullLabel = isAllDay ? $.lang[locale]['config.calendar.fulltime'] : fmtHMFromDate(ev.start, locale);
				const isGoogle = !!ev.extendedProps?.isGoogle;
				const noTitleText = (locale === 'ko') ? '(제목 없음)' : '(No title)';
				const rawTitle = (typeof ev.title === 'string' && ev.title) || (ev._def && typeof ev._def.title === 'string' && ev._def.title) || (ev.extendedProps && typeof ev.extendedProps.summary === 'string' && ev.extendedProps.summary) || '';
				const titleSafe = rawTitle.trim() !== 'undefined' ? rawTitle : noTitleText;
				const bgColor = (() => {
					const own = ev.backgroundColor || ev.borderColor || '';
					if (isGoogle) {
						return own || googleColorHex || '#4789e7';
					}
					return own || '#4789e7';
				})();
				...
				modalHtml += `
					...
					<div class="left-wrap">
						...
						data-color="${bgColor}"
						style="background-color:${bgColor}" readonly>
						<span>${isFullLabel}</span>
					</div>
					<div class="right-wrap">
						<div class="schedule-title">${titleSafe} ${isGoogle ? '' : ''}</div>
						<div class="schedule-time">${schedule_time}</div>
					</div>
					</li>
				`;
			}
		...
		function buildFcEvents(events) {
			function addDaysStr(ymd, days) {
				// ymd가 없거나 이상하면 오늘을 기준으로 보정
				...
				const dt = new Date((y || new Date().getFullYear()), (m ? m - 1 : 0), (d || 1));
			...
			// 허용: "9:05", "23:45", "23:45:00" (앞뒤 공백 허용)
			const TIME_RX = /^\s*\d{1,2}:\d{2}(?::\d{2})?\s*$/;
			const toHM = (t) => {
				if (!t || typeof t !== 'string') return '00:00';
				const parts = t.trim().split(':');   // ["HH","mm"] 또는 ["HH","mm","ss"]
				const Hn = parseInt(parts[0], 10);
				const Mn = parseInt(parts[1] ?? '0', 10);
				const HH = isFinite(Hn) ? String(Hn).padStart(2, '0') : '00';
				const MM = isFinite(Mn) ? String(Mn).padStart(2, '0') : '00';
				return `${HH}:${MM}`;
			};

			const mapped = (events || []).map(e => {
				let settings = {};
				try { settings = JSON.parse(e?.settings || '{}'); } catch {}

				const isHoliday = e?.seq && String(e.seq).startsWith('holiday_');

				// 날짜 확보: s_date/e_date → start/end → 오늘
				let sDate = (e?.s_date || '').slice(0, 10);
				let eDate = (e?.e_date || '').slice(0, 10);

				if (!sDate && typeof e?.start === 'string') sDate = e.start.slice(0, 10);
				if (!eDate && typeof e?.end   === 'string') eDate = e.end.slice(0, 10);
				if (!sDate) {
				const t = new Date();
				sDate = `${t.getFullYear()}-${String(t.getMonth() + 1).padStart(2, '0')}-${String(t.getDate()).padStart(2, '0')}`;
				}
				if (!eDate) eDate = sDate;

				// 시간 정규화
				const sHasTime = TIME_RX.test(e?.s_time || '');
				const eHasTime = TIME_RX.test(e?.e_time || '');
				const sTime = sHasTime ? toHM(e.s_time) : '00:00';
				const eTime = eHasTime ? toHM(e.e_time) : (sHasTime ? toHM(e.s_time) : '00:00');

				// fulltime 플래그
				const fullTFlag = (e?.fulltime === 'Y');

				// 시간이 둘 다 있으면 무조건 timed, 아니면 fullTFlag/날짜 범위로 all-day
				const isTimed = (sHasTime && eHasTime) && !fullTFlag;
				const fullT = !isTimed && (fullTFlag || sDate < eDate);

				// ── 공휴일(배경)
				if (isHoliday) {
				const endPlus1 = addDaysStr(eDate, 1);
				return {
					id: `holiday_${sDate}_${settings.title || e.title}`,
				...
				let startISO, endISO, allDayFlag;

				if (isTimed) {
					allDayFlag = false;
					startISO = `${sDate}T${sTime}`;
					endISO   = `${eDate}T${eTime}`;

					// Invalid Date 방지: 유효하지 않으면 보정
					let s = new Date(startISO);
					let t = new Date(endISO);
					if (isNaN(s.getTime())) s = new Date(`${sDate}T00:00`);
					if (isNaN(t.getTime())) t = new Date(`${eDate}T${eTime || '00:00'}`);

					// end < start → 동일 시각(0-duration)
					if (t.getTime() < s.getTime()) t = new Date(s.getTime());

					// 같은 날짜인데 end가 다음날 00:00 경계로 넘어가면 23:59로 클램프
					const sameYMD = (a, b) => a.getFullYear()===b.getFullYear() && a.getMonth()===b.getMonth() && a.getDate()===b.getDate();
					if (!sameYMD(s, t) && sDate === eDate && t.getHours() === 0 && t.getMinutes() === 0) {
						t = new Date(s); t.setHours(23, 59, 0, 0);
					}

					// (선택) 0-duration 렌더 꼬임 방지 +1분
					if (t.getTime() === s.getTime()) t = new Date(t.getTime() + 60 * 1000);
					...
					endISO = `${yy}-${mm}-${dd}T${HH}:${MI}`;
				} else {
					// all-day: end는 익스클루시브(+1일)
					allDayFlag = true;
					startISO = `${sDate}T00:00`;
					const endPlus1 = addDaysStr(eDate, 1);
					endISO   = `${endPlus1}T00:00`;
				}

				const isMulti = (sDate && eDate && sDate !== eDate) || (allDayFlag && sDate !== eDate);
			...
			// 인터페이스 유지
			// console.log(calendarElsettings)
			let pid = null;
			...
			const calendarFlag = (pid && calendarElsettings.get(pid) === true);
			return calendarFlag ? mapped : mapped.filter(ev => ev.classNames.includes('holiday-event'));
		}
		...
		function ymAdd(ym, delta) {
			...
			const yy = Math.floor(n / 12), mm = (n % 12) + 1;
			return `${yy}-${String(mm).padStart(2, '0')}`;
		}

		function monthsBetweenInclusive(sYmd, eYmd) {
			const out = [];
			const [sy, sm] = sYmd.split('-').map(Number);
			const [ey, em] = eYmd.split('-').map(Number);
			let y = sy, m = sm;
		...
		function invalidateAround(pid, ym) { // ym: 'YYYY-MM'
			const pidMap = monthCache.get(pid);
			if (!pidMap) return;
			...
			[prev, cur, next].forEach(d => {
				const key = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
				pidMap.delete(key);
			});
		}

		// 서버에서 내려온 3개월치 data를 달별로 분배해서 병합
		function upsertMonthCacheFromServer(pid, serverEvents) {
			if (!serverEvents) return;
			let pidMap = monthCache.get(pid);
			if (!pidMap) {
				pidMap = new Map();
				monthCache.set(pid, pidMap);
			}

			const mapped = buildFcEvents(serverEvents) || [];  // ★ 동일 포맷으로 맞춤
			const bucket = new Map(); // key: 'YYYY-MM' -> ev[]
			for (const ev of mapped) {
				// 시작/끝 계산 (all-day 및 00:00 경계 보정)
				const s0 = new Date(ev.start);
				const e0 = new Date(ev.end);
				const s  = new Date(s0);
				const e  = new Date(e0);

				// 종일(Y) 또는 end가 00:00:00인 경우 표시용 inclusive 보정
				if (ev.fulltime === 'Y' || ev.allDay) {
				e.setDate(e.getDate() - 1);
				} else if (e0.getHours() === 0 && e0.getMinutes() === 0 && e0.getSeconds() === 0) {
			...
			for (const [key, arr] of bucket.entries()) {
				const prev = pidMap.get(key) || [];
				const seen = new Set();
				const merged = [];
				for (const item of [...prev, ...arr]) {
				const k = `${item.id || item.seq || item.title}|${item.start}|${item.end}|${item.allDay ? 1 : 0}`;
				if (!seen.has(k)) { seen.add(k); merged.push(item); }
				}
				pidMap.set(key, merged);
			}
		}
		...
		function monthsAround(ym) {
			...
			const cur  = new Date(y, m - 1, 1);
			const next = new Date(y, m, 1);
			return [prev, cur, next].map(d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`);
		}

		// 전/당/다음 달 캐시 무효화
		function invalidateAround(pid, ym) {
			const pidMap = monthCache.get(pid);
			if (!pidMap) return;

			const [y, m] = ym.split('-').map(Number);
			const prev = new Date(y, m - 2, 1);  // 전월
			const cur  = new Date(y, m - 1, 1);  // 당월
			const next = new Date(y, m, 1);      // 다음월
		...
		function fetchMonth(pid, yearMonth, data_type) {
			let pidCache = monthCache.get(pid);
			let pidInflight = inflight.get(pid);
			if (!pidCache) { pidCache = new Map(); monthCache.set(pid, pidCache); }
			if (!pidInflight) { pidInflight = new Map(); inflight.set(pid, pidInflight); }

			// 이미 캐시되어 있으면 재사용
			if (pidCache.has(yearMonth)) return Promise.resolve(pidCache.get(yearMonth));
			// 동일 YM에 대한 진행중 요청이 있으면 그것을 공유
			if (pidInflight.has(yearMonth)) return pidInflight.get(yearMonth);

			const prevYM = ymAdd(yearMonth, -1);
			const nextYM = ymAdd(yearMonth, +1);
			...
			const dedupe = (list) => {
				...
				const k = `${ev.id || ev.seq || ev.title}|${ev.start}|${ev.end}|${ev.allDay ? 1 : 0}`;
				if (!seen.has(k)) { seen.add(k); out.push(ev); }
				}
				return out;
			};
			const mergeSet = (cache, ym, arr) => {
				const prev = cache.get(ym) || [];
				const merged = dedupe(prev.concat(arr || []));
				cache.set(ym, merged);
			};
			...
			const p = new Promise((resolve, reject) => {
				...
				success: function(raw) {
					...
					for (const ev of all) {
					const sStr = ev.start_date || (ev.start?.slice(0, 10)) || '';
					let eStr   = ev.end_date   || '';

					if (!sStr) continue;

					// end 보정 (종일: exclusive → inclusive / 00:00 경계 보정)
					if (!eStr) {
						if (ev.allDay && ev.end) eStr = addDaysStr(ev.end.slice(0, 10), -1);
						else if (ev.end)         eStr = ev.end.slice(0, 10);
						else                     eStr = sStr;
					} else {
						// 선택: 서버 end_date가 'YYYY-MM-DD' 00:00:00 의미라면 하루 빼기
						// 만약 서버 end_date가 이미 inclusive라면 이 블록은 제거하세요.
						// (현재 로직은 allDay에서만 -1 처리하므로 그대로 두는 게 안전)
					}

					// 🔹 s~e 사이 모든 YM 버킷에 푸시
					const sYM = sStr.slice(0, 7);
					const eYM = eStr.slice(0, 7);
					for (const ym of monthsBetweenInclusive(sYM, eYM)) {
						put(ym, ev);
					}
					}

					// 🔹 set → mergeSet 로 변경 (덮어쓰기 금지)
					mergeSet(pidCache, prevYM,   buckets.get(prevYM));
					mergeSet(pidCache, yearMonth, buckets.get(yearMonth));
					mergeSet(pidCache, nextYM,   buckets.get(nextYM));

					// inflight 해제 (요청 공유했던 3개월 키 모두 제거)
					pidInflight.delete(prevYM);
					pidInflight.delete(yearMonth);
					pidInflight.delete(nextYM);

					resolve(pidCache.get(yearMonth));
				},
			...
			// 동일 요청 공유 (3개월 모두 같은 Promise 공유)
			pidInflight.set(prevYM, p);
			pidInflight.set(yearMonth, p);
			pidInflight.set(nextYM, p);
		...
		function initCalendar(pid, c, retryCount = 0) {
			...
			const color = elsettings.google_color ? elsettings.google_color.trim() : '#4789e7';
			googleColor.set(pid, color);
			// console.log(`pid:${pid} => color:${color}`);
			...
			//구글캘린더 연동
			const googleShow = (calendarElsettings.get(pid) === true);
			const eventSources = [
				function(fetchInfo, success, failure) {
					...
					Promise.all(
						keys.map(k => fetchMonth(pid, k, data_type))
					)
					.then(() => {
			...
			// ✅ 2️⃣ 조건문으로 구글 캘린더/공휴일 캘린더를 추가
			const userCalendarId = elsettings.google_id; // 사용자 입력 또는 DB값
			const google_color = elsettings.google_color;
			const holidayCalendarId = 'ko.south_korea#holiday@group.v.calendar.google.com';
			const apiKey = elsettings.google_api;
			const googleSyncOn = String(elsettings.google_sync || '0') === '1';
			...
			if (googleSyncOn && userCalendarId && userCalendarId.trim() !== '' && apiKey && googleShow) {
				eventSources.push({
					// 🔹 gcal 플러그인 대신 직접 Google Calendar API 호출
					events: function(fetchInfo, success, failure) {
						...
						const url = `https://www.googleapis.com/calendar/v3/calendars/${
							encodeURIComponent(userCalendarId)
						...
						fetch(url)
							...
							.then(data => {
								...
								const googleColorHex = google_color || '#4789e7';

								const fcEvents = items
									.filter(ev => ev.visibility !== 'private')  // 비공개는 아예 제외
									.map(ev => {
										const isAllDay = !!ev.start.date;
										const startStr = ev.start.dateTime || ev.start.date;
										const endStr   = (ev.end && (ev.end.dateTime || ev.end.date)) || startStr;
										...
										if (isAllDay) {
											...
											end_date = end ? fmtDate(end) : start_date;
											s_time   = fmtHHMM(start);
											e_time   = end ? fmtHHMM(end) : '';
										}
										}

										// 🔹 제목 없는 이벤트 처리
										const hasTitle = ev.summary && ev.summary.trim().length > 0;
										const title    = hasTitle ? ev.summary.trim() : (language == 'ko') ? '(제목 없음)' : '(No title)';

										// description → memo
										memo = (ev.description && typeof ev.description === 'string')
										? ev.description
										: '';
										...
										return {
											id: ev.id,
											title,                 // ✅ 여기서 (제목 없음)까지 세팅
											start: startStr,
											end: endStr,
											allDay: isAllDay,
											backgroundColor: googleColorHex,
											borderColor: googleColorHex,
											extendedProps: {
												...ev,
												isGoogle: true,
												start_date,
			...
			var calendar = new FullCalendar.Calendar(calendarEl, {
				...
				eventDidMount: function(info) {
					...
					if (info.event.extendedProps?.isGoogle) {
						const col = (googleColor && googleColor.get(pid)) || '';
						if (col) {
							info.el.style.backgroundColor = col;
							info.el.style.borderColor = col;

							// 월간 뷰의 작은 점에도 반영 (dot 모드일 경우)
				...
				eventClick: function(info) {
					...
					const eventsOnSameDay = allEvents.filter(ev => {
						...
						if (isGoogle) {
							// ---- 구글 이벤트 전용 날짜 계산 (로컬 기준) ----
							const s = ev.start ? new Date(ev.start) : null;
							let   e = ev.end   ? new Date(ev.end)   : null;

							if (!s) return false;

							// end 없으면 start와 같은 날로 취급
							if (!e) e = new Date(s);

							// allDay 인 경우, FullCalendar는 end를 "exclusive"로 쓰므로 마지막 날 = end - 1일
							if (ev.allDay && ev.end) {
								e.setDate(e.getDate() - 1);
							}

							const startStr = toYMDLocal(s);   // ✅ 로컬 날짜
							const endStr   = toYMDLocal(e);   // ✅ 로컬 날짜

							return (clickedDateStr >= startStr && clickedDateStr <= endStr);
						}

						// ---- 기존(내부) 이벤트 로직 그대로 유지, 날짜 계산만 toYMDLocal로 정리 ----
						const startStrBase = (ev.extendedProps.start_date || ev.startStr || '').split('T')[0];

						let startStr = startStrBase;
						if (!startStr && ev.start) {
							startStr = toYMDLocal(ev.start);   // 안전장치
						}
						...
						if (ev.extendedProps.end_date) {
							...
							if (ev.end) {
								...
								if (dayDiff >= 2) {
									const d = new Date(ev.end);
									d.setDate(d.getDate() - 1);
									endInclStr = toYMDLocal(d);     // ✅ 여기서도 toISOString 대신 로컬
								} else {
									endInclStr = startStr;
								}
							} else {
							...
							// timed 이벤트는 endStr도 로컬 기준으로
							endInclStr = toYMDLocal(ev.end);
						} else {
							endInclStr = startStr;
						}
				...
				eventContent: function(arg) {
					...
					if (arg.event.classNames.includes('holiday-event')) {
						if (locale === 'ko') {
							const titleText = (arg.event.title && arg.event.title.trim() !== '') ? arg.event.title : noTitleText;
							return { html: `<div class="holiday-label">${titleText}</div>` };
						} else {
							return { html: '' };
						}
					}
					...
					const rawTitle = (typeof arg.event.title === 'string') ? arg.event.title : '';
					const titleText = rawTitle.trim() !== 'undefined' ? rawTitle : noTitleText;
			...
			calendar.on('datesSet', (arg) => { //월 초기 노출 개수
				// ▼ 부족 월 프리페치 (prev/next 이동 시 캐시에 없는 달만 보충)
				if (!calendar.__prefetching) {
					const keys = monthsInRange(arg.start, arg.end); // ['YYYY-MM', ...]
					const pidMap = monthCache.get(pid) || new Map();
					const missing = keys.filter(k => !pidMap.has(k));

					if (missing.length) {
						calendar.__prefetching = true;
						const dt = ($('.userEL'+pid).attr('data-modoo-type')) || '';
						Promise.all(missing.map(k => fetchMonth(pid, k, dt)))
						.then(() => arg.view.calendar.refetchEvents())
						.finally(() => { calendar.__prefetching = false; });
					}
				}
			});
				...
				if(property.ISLOCK == 'false') {
					if(property.COUNT==0) {
						...
						if(b.VALIDPLAN && (b.VALIDTYPE == 'BN' || b.VALIDTYPE == 'SM')) {
							...
							if(typeof b.SETTINGS.qm_onoff != 'undefined' && b.SETTINGS.qm_onoff==true) {
								var quick_type = ($('body').width() > 768) ? 'pc' : 'mobile';
								if(!(b.PARENT.page != null && (quick_type=='mobile'))) $.quickmenu.init(b.SID, b.PUBLISH,quick_type);
							}
						}

						if(b.VALIDPLAN && b.VALIDTYPE != 'PK' && typeof b.SETTINGS.viewportMode != 'undefined' && b.SETTINGS.vpMode_onoff===true) {
							console.log($('.el-footer').length)
							$.mpcWeb.mpcWebhtml(vpmode,'',b.SETTINGS.zoom);
							mobileWebfnavCheck(vpmode);
						}
		...
		if(typeof data.sync !='undefined' && typeof data.sync.list != 'undefined' && (data.sync.list).length) {
			...
			$.each(data.sync.list, function(i,v) {
				...
				var imgbg_check = (typeof $item.find('.img-bg').attr('style') == 'undefined') ? false : true,
					href="./index.html" + v.page + '/view/' + v.seq,
...
var mobileProductBtnPosition = function(property,device_type) {
	console.log('mobileProductBtnPosition')
	var quick_height = $(`.quick-menu-${property.SID}.quickmenu`).innerHeight();

	if(device_type=='mobile') {
		var quick_height = $(`.quick-menu-${property.SID}.quickmenu`).innerHeight(),
			productBtn_height = $('.m-product-btn').innerHeight(),
			productBtn_position = ($.quickmenu['quickmenu']['img_type'][device_type]=='bottom_default') ? quick_height : '';
console.log(productBtn_position);
		if($.quickmenu['quickmenu']['img_type'][device_type]=='bottom_default') $('.m-product-btn, .product-detail-btn').css('bottom',productBtn_position);
		else $(`.quick-menu-${property.SID}.quickmenu`).css('bottom',productBtn_height+30);
